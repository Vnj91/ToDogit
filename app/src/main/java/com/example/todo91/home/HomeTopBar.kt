...
fun HomeTopBar(
    ...
    onSearchClose: () -> Unit,
    currentSortOrder: com.example.todo91.model.SortOrder,
    ...
    TopAppBar(
        title = { if (!isSearchActive) Text("Notes") },
        navigationIcon = {
            if (isSearchActive) {
                IconButton(onClick = onSearchClose) { Icon(Icons.AutoMirrored.Filled.ArrowBack, contentDescription = "Close Search") }
            } else {
                IconButton(onClick = onMenuClick) { Icon(Icons.Default.Menu, contentDescription = "Open Drawer") }
            }
        },
        actions = {
            if (isSearchActive) {
                TextField(
                    ...
                    placeholder = { Text("Search your notes...") }, modifier = Modifier.weight(1f), singleLine = true,
                ...
                IconButton(onClick = { onSearchQueryChange("") }) { Icon(Icons.Filled.Close, contentDescription = "Clear Search") }
            } else {
                IconButton(onClick = onSearchClick) { Icon(Icons.Filled.Search, contentDescription = "Search Notes") }
                IconButton(onClick = onFilterClick) { Icon(Icons.Filled.FilterList, contentDescription = "Filter Notes") }
                DropdownMenu(expanded = showFilterMenu, onDismissRequest = onDismissFilterMenu) {
                    FilterOrder.values().forEach { filterOrder ->
                        DropdownMenuItem(text = { Text(filterOrder.name.capitalizeWords()) },
                            onClick = { onSelectFilter(filterOrder); onDismissFilterMenu() }
                        )
                    }
                }
                IconButton(onClick = { showSortMenu = true }) { Icon(Icons.Filled.Sort, contentDescription = "Sort Notes") }
                DropdownMenu(expanded = showSortMenu, onDismissRequest = { showSortMenu = false }) {
                    com.example.todo91.model.SortOrder.values().forEach { sortOrder ->
                        DropdownMenuItem(text = { Text(sortOrder.name.capitalizeWords()) },
                            onClick = { onSortClick(sortOrder); showSortMenu = false }, enabled = sortOrder != currentSortOrder
                        )
                    }
                }
            }
